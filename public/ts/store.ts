const store = Vue.reactive({
  ...
  selectChannel(channelId) {
    ...
    if (ch) ch.unread = 0;
    if (window.innerWidth <= 540) this.sidebarOpen = false;
  },
  ...
  addContact(profile) {
    if (this.contacts.find(c => c.id === profile.id)) return false;
  ...
  addFriendRequest(req) {
    if (this.friendRequests.find(r => r.id === req.id)) return false;
