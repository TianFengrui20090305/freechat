const ChatPage = {
  ...
  computed: {
    ...
    filteredChannels() {
      if (!this.searchQuery) return this.store.channels;
      const q = this.searchQuery.toLowerCase();
      return this.store.channels.filter(c => c.name.toLowerCase().includes(q));
    },
    channelGroups() {
      if (!this.store.channels) return [];
    ...
    groupedMessages() {
      ...
      for (const msg of msgs) {
        ...
        if (d !== cd) { if (ci.length) groups.push({ date: cd, items: ci }); cd = d; ci = []; }
        ci.push(msg);
      }
      if (ci.length) groups.push({ date: cd, items: ci });
      return groups;
    },
    profileAvatarStyle() {
    ...
    friendAddedTimeText() {
      if (!this.profileUser) return '';
      const meta = this.friendMeta[this.profileUser.id];
      if (!meta?.addedTime) return '未知';
      return this.fmtFullDate(meta.addedTime);
    },
    searchedAvatarStyle() {
      if (!this.contactSearchResult) return {};
      const colors = ['#4f7cff','#2ed573','#ff4757','#ffa502','#a55eea','#1e90ff','#ff6b81','#7bed9f'];
      let h = 0;
      for (let i = 0; i < this.contactSearchResult.id.length; i++) h = ((h << 5) - h) + this.contactSearchResult.id.charCodeAt(i);
      return { background: colors[Math.abs(h) % colors.length] };
    },
  },
  methods: {
    ...
    joinChannel() {
      ...
      if (!id) return;
      if (this.store.channels.find(c => c.id === id)) { this.store.selectChannel(id); this.showChannelModal_ = false; return; }
      ...
      if (!this.store.messages[id]) this.store.messages[id] = [];
    ...
    joinChannelById(ch) {
      ...
      if (!this.store.messages[ch.id]) this.store.messages[ch.id] = [];
    ...
    async showProfile(userId, event) {
      ...
      this.$nextTick(() => {
        ...
        if (rect) {
          const pw = 320; let l = rect.left + 20, t = rect.top - 10;
          if (l + pw > window.innerWidth - 20) l = window.innerWidth - pw - 20;
          if (t < 10) t = 10;
    ...
    startPrivateChat() {
      ...
      let ch = this.store.channels.find(c => c.id === pid);
      if (!ch) { ch = { id: pid, name: this.profileUser.username, type: 'private', unread: 0, icon: '👤' }; this.store.channels.push(ch); if (!this.store.messages[pid]) this.store.messages[pid] = []; }
    ...
    // ===== Friend Tags / Notes =====
    saveFriendData() { /* auto-bound via v-model, could persist to localStorage later */ },
    addTag(userId) {
      const tag = this.tagInput.trim();
      if (!tag) return;
      if (!this.friendTags[userId]) this.friendTags[userId] = [];
      if (this.friendTags[userId].includes(tag)) return;
      this.friendTags[userId].push(tag);
      this.tagInput = '';
    },
    removeTag(userId, idx) { if (this.friendTags[userId]) this.friendTags[userId].splice(idx, 1); },
    removeLastTag(userId) {
      if (this.tagInput) return;
      const tags = this.friendTags[userId];
      if (tags && tags.length) tags.pop();
    },

    // ===== Messaging =====
    send() { const t = this.inputText.trim(); if (!t || !this.store.currentChannelId) return; wsManager.send(this.store.currentChannelId, t); this.inputText = ''; },
    ...
    goDetails() {
      const ch = this.store.currentChannel;
      if (ch) this.$router.push('/details/' + encodeURIComponent(ch.id));
    },

    // ===== Utils =====
    fmtTime(ts) { const d = new Date(ts); return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; },
    fmtDate(ts) { const d = new Date(ts); const n = new Date(); if (d.toDateString() === n.toDateString()) return '今天 ' + this.fmtTime(ts); return `${d.getMonth()+1}/${d.getDate()} ` + this.fmtTime(ts); },
    ...
    scrollDown() { this.$nextTick(() => { const el = this.$refs.msgArea; if (el) el.scrollTop = el.scrollHeight; }); },

    // ===== Drag & Drop =====
    onDragOver(e) {
      const msgArea = this.$refs.msgArea;
      const inputBar = document.querySelector('.input-bar');
      if (!msgArea || !inputBar) { this.dragZone = null; return; }
    ...
    async sendFiles(files) {
      const file = files[0];
      if (!file) return;
      if (!confirm(`确认发送文件？\n\n文件名：${file.name}\n大小：${(file.size / 1024 / 1024).toFixed(1)} MB`)) return;
      try {
  ...
  mounted() {
    ...
    if (fid) { this.store.selectChannel(fid); wsManager.connect(fid); }
    this.scrollDown();
